...
export default function ObrDiario({params}: any){
    ...
    return(
        <div className="flex flex-col w-full h-screen items-center bg-slate-900">
            ...
            <div className='flex flex-row w-full h-10 items-center justify-between p-2'>
                <div className='text-sky-500 text-xs font-bold mt-3 mb-3'>
                    Diarios:
                </div>
                <Link className="flex items-center justify-center bg-sky-700 hover:bg-sky-600 rounded-lg w-30 h-6 p-2 cursor-pointer" href={`/NewDiario/${params.obrId}`}>
            ...
            <div className='flex flex-col w-full h-full p-1'>
                {diarios?.map((row:any, idx) => {
                    return <button key={idx} >
                        <div className="flex flex-col w-full h-auto bg-slate-800 mb-1">
                            <div className='flex w-full flex-row items-start justify-between px-2'>
                                ...
                                <span className="text-xs text-white font-bold w-[50%] mt-1">{Intl.DateTimeFormat('pt-BR', {timeZone: 'UTC'}).format(new Date(row.diaData))}</span>
