...
export default function AltCargo({params}: any){
    const router = useRouter();

    const [crgId, setCrgId] = useState('');
    const [crgDescricao, setCrgDescricao] = useState('');
    
    useEffect(() => {   
        const idCrg = params.crgId;    

        api.get(`/searchCargo/${idCrg}`).then(resp => {
            setCrgId(resp.data[0].crgId);
            setCrgDescricao(resp.data[0].crgDescricao);
        }).catch((err) => {
    ...
    async function handleCadastra(e:any){      
        ...
        api({
          ...
          data: {
            ...
            alert(`Cargo alterado com sucesso! ${response.data}` )
        ...
        <div className="flex flex-col w-full h-auto items-center bg-slate-600">
            ...
            <div className="flex flex-col w-full h-auto  items-center p-2 md:p-20">
                <form className="flex flex-col justify-center items-center bg-white p-12 rounded-lg w-full md:w-4/5 h-full max-w-full gap-2 ">
                    <h2 className="text-blue-700 text-2xl font-bold mb-6 ">Altere dados do Cargo</h2>
                    <div className="flex flex-row justify-between items-center w-full h-auto gap-2">
                        <input className="w-full border-2 border-black p-3 mb-3 focus:border-blue-700" 
                            name="descAtiv" placeholder="Informe Cargo" type="text" value={crgDescricao} onChange={(e) => {setCrgDescricao(e.target.value)}} />
                    </div>                    
